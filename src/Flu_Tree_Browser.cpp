// $Id: Flu_Tree_Browser.cpp,v 1.5 2013/01/23 01:00:07 kpettit1 Exp $
...
void Flu_Tree_Browser :: NodeList :: insert_at(Flu_Tree_Browser::Node* i, Flu_Tree_Browser::Node* n)
{
	int		c;
	...
	{
		// Find index of i within the list
		for( c = 0; c < _nNodes; c++ )
			if (_nodes[c] == i)
			{
				add( n, c);
...
#endif
{
  ...
  rdata.nextId = 1;
  rdata.searchIndex = 1;
  rdata.defaultCollapseIcons[0] = new Fl_Pixmap( plus_xpm );
  rdata.defaultCollapseIcons[1] = new Fl_Pixmap( minus_xpm );
  rdata.defaultBranchIcons[0] = new Fl_Pixmap( folder_closed_xpm );
  rdata.defaultBranchIcons[1] = new Fl_Pixmap( folder_open_xpm );
...
void Flu_Tree_Browser :: on_dnd_leave()
{
  ...
  if( scrollH->visible() )
    {
      float max = (float) (0.01f * (scrollH->maximum() - scrollH->minimum()));
  ...
  if( scrollV->visible() )
    {
      float max = (float) (0.01f * (scrollV->maximum() - scrollV->minimum()));
      if( max < 10.0f ) max = 10.0f;
      if( autoScrollY > 0.0f )
	autoScrollY = max;
      else
  ...
  if( scrollH->visible() )
    {
      ...
      float min = 1.0f, max = (float) (0.01f * (scrollH->maximum() - scrollH->minimum()));
      if( max < 10.0f ) max = 10.0f;
      if( X < (x()+30) ) // left
	{
  ...
  if( scrollV->visible() )
    {
      ...
      float min = 1.0f, max = (float) (0.01f * (scrollV->maximum() - scrollV->minimum()));
      if( max < 10.0f ) max = 10.0f;
      if( Y < (y()+30) ) // top
	{
...
Flu_Tree_Browser :: Node :: Node( bool l, const char* n, Node *p, RData &rdata, Fl_Widget *w, bool showLbl )
{
  ...
  if( is_leaf() )
    {
      lIcon = rdata.leafIcon;
      textColor = rdata.defLeafColor;
      textFont = rdata.defLeafFont;
      textSize = rdata.defLeafSize;
    }
  else
    {
      cIcon[0] = rdata.collapseIcons[0];
      cIcon[1] = rdata.collapseIcons[1];
      bIcon[0] = rdata.branchIcons[0];
      bIcon[1] = rdata.branchIcons[1];
      textColor = rdata.defBranchColor;
      textFont = rdata.defBranchFont;
      textSize = rdata.defBranchSize;
    }
...
void Flu_Tree_Browser :: Node :: open( bool b )
{
  ...
  if( tree->rdata.animate )
      ...
      if( !tree->rdata.animating && !tree->rdata.animatedNode )
	{
	  ...
	  tree->rdata.animationOffset = (float) (b ? -totalChildH : -1);
...
Flu_Tree_Browser::Node* Flu_Tree_Browser::insert_at(Flu_Tree_Browser::Node* p, Flu_Tree_Browser::Node* i,
													const char *name, Fl_Widget* w, bool showLabel)
{
	if (p)
		return p->insert_at(p, i, name, rdata, w, showLabel);
	return 0;
}
...
Flu_Tree_Browser::Node* Flu_Tree_Browser :: Node :: insert_at( Node* p, Node* i, const char* path, 
											RData &rdata, Fl_Widget *w, bool showLabel )
{
  ...
  ///////////// process the node ///////////////////
	...
	if( !branchNode )
	    ...
	    retNode = new Node( false, nodeName, this, rdata, w, showLabel );

	// Find number of the given insert node
	_children.insert_at(i, retNode);
	rdata.forceResize = true;
	rdata.visibilityChanged = true;
