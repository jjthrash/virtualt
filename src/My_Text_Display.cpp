...
void cb_cursor_blink(void *pOpaque)
{
	My_Text_Display* pDisp = (My_Text_Display *) pOpaque;
...
My_Text_Display::~My_Text_Display() {
	Fl::remove_timeout(cb_cursor_blink, this);
  if (mBuffer) mBuffer->remove_modify_callback(buffer_modified_cb, this);
  if (mLineStarts) delete[] mLineStarts;
}

/*
** Returns the position of the specified line, or -1 if it doesn't exist)
*/
int My_Text_Display::get_line_pos(int line)
{
  int startPos, lineEnd, nextLineStart, lineCount;
...
*/
void My_Text_Display::buffer( Fl_Text_Buffer *buf ) {
...
*/
void
My_Text_Display::highlight_data(Fl_Text_Buffer *styleBuffer,
...
int My_Text_Display::longest_vline() {
  int longest = 0;
...
void My_Text_Display::resize(int X, int Y, int W, int H) 
{
  ...
  text_area.w = W-LEFT_MARGIN-RIGHT_MARGIN*2-mLeftMargin;
  text_area.h = H-TOP_MARGIN-BOTTOM_MARGIN;
  int i;

  /* Find the new maximum font height for this text display */
  for (i = 0, mMaxsize = 2+fl_height(textfont(), textsize()); i < mNStyles; i++)
    ...
    if (scrollbar_width()) {
          ...
          mNBufferLines >= mNVisibleLines - 1)
      {
        ...
        if (scrollbar_align() & FL_ALIGN_LEFT) {
          ...
          mVScrollBar->resize(X+W-1-scrollbar_width(), text_area.y-TOP_MARGIN,
	 ...
	 don't turn it off (ie mix both of your solutions). */
          ...
          (mVScrollBar->visible() || longest_vline() > text_area.w))
      {
        ...
        if (scrollbar_align() & FL_ALIGN_TOP) {
          ...
          mHScrollBar->resize(text_area.x-LEFT_MARGIN-mLeftMargin, Y,
                              text_area.w+LEFT_MARGIN+RIGHT_MARGIN*2+mLeftMargin, scrollbar_width());
        } else {
          text_area.y = Y+TOP_MARGIN;
          text_area.h = H - scrollbar_width()-TOP_MARGIN-BOTTOM_MARGIN;
          mHScrollBar->resize(text_area.x-LEFT_MARGIN-mLeftMargin, Y+H-scrollbar_width(),
                              text_area.w+LEFT_MARGIN+RIGHT_MARGIN*2+mLeftMargin, scrollbar_width());
        }
      }
    }
...
*/
void My_Text_Display::draw_text( int left, int top, int width, int height ) {
  ...
  fl_push_clip( left, top, width, height );
	  Fl_Region r = XRectangleRegion(left, top, width, height); 
	  fl_clip_region(r);
...
void My_Text_Display::redisplay_range(int startpos, int endpos) {
...
*/
void My_Text_Display::draw_range(int startpos, int endpos) {
...
*/
void My_Text_Display::insert_position( int newPos ) {
  ...
  redisplay_range(mCursorPos - 1, mCursorPos + 1);
  int line;
  position_to_linecol(mCursorPos, &line, &mCurrentCol);

}

void My_Text_Display::show_cursor(int b) {
  mCursorOn = b;
  redisplay_range(mCursorPos - 1, mCursorPos + 1);
}

void My_Text_Display::cursor_style(int style) {
  mCursorStyle = style;
  if (mCursorOn) show_cursor();
}

void My_Text_Display::wrap_mode(int wrap, int wrapMargin) {
...
*/
void My_Text_Display::insert(const char* text) 
{
  int pos = mCursorPos;
  int line;
...
** cursor location.
*/
void My_Text_Display::overstrike(const char* text) {
...
int My_Text_Display::position_to_xy( int pos, int* X, int* Y ) {
...
*/
int My_Text_Display::position_to_linecol( int pos, int* lineNum, int* column ) {
  int retVal;
...
*/
int My_Text_Display::in_selection( int X, int Y ) {
...
*/
int My_Text_Display::wrapped_column(int row, int column) {
...
*/
int My_Text_Display::wrapped_row(int row) {
    if (!mContinuousWrap || row < 0 || row > mNVisibleLines)
    	return row;
...
*/
void My_Text_Display::display_insert() {
    ...
    if (insert_position() >= lastChar)
      topLine
        += count_lines(lastChar - (wrap_uses_character(mLastChar) ? 0 : 1),
                        insert_position(), false) + mNVisibleLines/2;
  }
...
void My_Text_Display::show_insert_position() {
...
*/
int My_Text_Display::move_right() {
...
*/
int My_Text_Display::count_lines(int startPos, int endPos,
...
** by avoiding the additional step of scanning back to the last newline.
*/
int My_Text_Display::skip_lines(int startPos, int nLines,
    bool startPosIsLineStart) {
    int retLines, retPos, retLineStart, retLineEnd;
...
** visLineLength.
*/
int My_Text_Display::line_end(int pos, bool startPosIsLineStart) {
    int retLines, retPos, retLineStart, retLineEnd;
...
*/
int My_Text_Display::line_start(int pos) {
    int retLines, retPos, retLineStart, retLineEnd;
...
*/
int My_Text_Display::rewind_lines(int startPos, int nLines) {
...
void My_Text_Display::next_word() {
...
void My_Text_Display::previous_word() {
  int pos = insert_position();
  pos--;
...
*/
void My_Text_Display::buffer_predelete_cb(int pos, int nDeleted, void *cbArg) {
    My_Text_Display *textD = (My_Text_Display *)cbArg;
...
*/
void My_Text_Display::buffer_modified_cb( int pos, int nInserted, int nDeleted,
...
*/
void My_Text_Display::maintain_absolute_top_line_number(int state) {
...
*/
int My_Text_Display::get_absolute_top_line_number() {
    if (!mContinuousWrap)
	return mTopLineNum;
    if (maintaining_absolute_top_line_number())
...
*/
void My_Text_Display::absolute_top_line_number(int oldFirstChar) {
    if (maintaining_absolute_top_line_number()) {
...
*/
int My_Text_Display::maintaining_absolute_top_line_number() {
...
*/
void My_Text_Display::reset_absolute_top_line_number() {
    mAbsTopLineNum = 1;
...
*/
int My_Text_Display::position_to_line( int pos, int *lineNum ) {
  int i;
    ...
    if ( empty_vlines() ) {
      if ( mLastChar < mBuffer->length() ) {
        if ( !position_to_line( mLastChar, lineNum ) ) {
          Fl::error("My_Text_Display::position_to_line(): Consistency check ptvl failed");
          return 0;
        }
...
*/
void My_Text_Display::draw_vline(int visLineNum, int leftClip, int rightClip,
  ...
  if ( stdCharWidth <= 0 ) {
    Fl::error("My_Text_Display::draw_vline(): bad font measurement");
    free((void *)lineStr);
    return;
  }
...
*/
void My_Text_Display::draw_string( int style, int X, int Y, int toX,
...
*/
void My_Text_Display::clear_rect( int style, int X, int Y,
...
*/
void My_Text_Display::draw_cursor( int X, int Y ) {
  typedef struct {
    int x1, y1, x2, y2;
  }
  Segment;
...
** be more appropriate.
*/
int My_Text_Display::position_style( int lineStartPos,
...
*/
int My_Text_Display::string_width( const char *string, int length, int style ) {
  Fl_Font font;
  int fsize;
...
** closest to (X, Y).
*/
int My_Text_Display::xy_to_position( int X, int Y, int posType ) {
...
*/
void My_Text_Display::xy_to_rowcol( int X, int Y, int *row,
...
*/
void My_Text_Display::offset_line_starts( int newTopLineNum ) {
...
*/
void My_Text_Display::update_line_starts( int pos, int charsInserted,
...
*/
void My_Text_Display::calc_line_starts( int startLine, int endLine ) {
...
/*
** Given a My_Text_Display with a complete, up-to-date lineStarts array, update
** the lastChar entry to point to the last buffer position displayed.
*/
void My_Text_Display::calc_last_char() {
  int i;
  for (i = mNVisibleLines - 1; i >= 0 && mLineStarts[i] == -1; i--) ;
  mLastChar = i < 0 ? 0 : line_end(mLineStarts[i], true);
}

void My_Text_Display::scroll(int topLineNum, int horizOffset) {
  mTopLineNumHint = topLineNum;
  mHorizOffsetHint = horizOffset;
  resize(x(), y(), w(), h());
}

void My_Text_Display::scroll_(int topLineNum, int horizOffset) {
...
*/
void My_Text_Display::update_v_scrollbar() {
...
*/
void My_Text_Display::update_h_scrollbar() {
...
*/
void My_Text_Display::v_scrollbar_cb(Fl_Scrollbar* b, My_Text_Display* textD) {
  if (b->value() == textD->mTopLineNum) return;
  textD->scroll(b->value(), textD->mHorizOffset);
}

void My_Text_Display::h_scrollbar_cb(Fl_Scrollbar* b, My_Text_Display* textD) {
...
*/
void My_Text_Display::draw_line_numbers(bool clearAll) {
#if 0
...
*/
int My_Text_Display::measure_vline( int visLineNum ) {
...
*/
int My_Text_Display::empty_vlines() {
...
*/
int My_Text_Display::vline_length( int visLineNum ) {
...
*/
void My_Text_Display::find_wrap_range(const char *deletedText, int pos,
...
** efficiently).
*/
void My_Text_Display::measure_deleted_lines(int pos, int nDeleted) {
...
*/
void My_Text_Display::wrapped_line_counter(Fl_Text_Buffer *buf, int startPos,
...
*/
int My_Text_Display::measure_proportional_character(char c, int colNum, int pos) {
    int charLen, style;
...
*/
void My_Text_Display::find_line_end(int startPos, bool startPosIsLineStart,
...
*/ 
int My_Text_Display::wrap_uses_character(int lineEndPos) {
    char c;
...
*/
void My_Text_Display::extend_range_for_styles( int *startpos, int *endpos ) {
  ...
  /* The peculiar protocol used here is that modifications to the style
     ...
     the text buffer BEFORE My_Text_Display.c's modify callback, so that it can keep
     the style buffer in step with the text buffer.  The style-update
     callback can't just call for a redraw, because My_Text_Display hasn't processed
...
void My_Text_Display::draw(void) {
...
//	  Fl_Region r = XRectangleRegion(0, 0, w(), h());
	...
	{
	    fl_rectf(text_area.x-mLeftMargin - LEFT_MARGIN-1, text_area.y-TOP_MARGIN-1,
             mLeftMargin, text_area.h+TOP_MARGIN+BOTTOM_MARGIN, mUtilityMarginColor);
	    fl_rectf(text_area.x - LEFT_MARGIN, text_area.y-1,
             LEFT_MARGIN, text_area.h+2, color());
	}
	else
	    fl_rectf(text_area.x-mLeftMargin, text_area.y-TOP_MARGIN,
             mLeftMargin, text_area.h+TOP_MARGIN+BOTTOM_MARGIN, color());

    // right margin
    fl_rectf(text_area.x+text_area.w, text_area.y-TOP_MARGIN,
             RIGHT_MARGIN + scrollbar_width(), text_area.h+TOP_MARGIN+BOTTOM_MARGIN, 
			 color());
	...
	if (bHasUtilityMargin)
	{
	    fl_rectf(0, text_area.y-TOP_MARGIN-1,
             mLeftMargin, text_area.h+TOP_MARGIN+BOTTOM_MARGIN, mUtilityMarginColor);
	    fl_rectf(text_area.x - LEFT_MARGIN, text_area.y-1,
             LEFT_MARGIN, text_area.h+2, color());
	}
	    ...
	    fl_rectf(text_area.x-mLeftMargin, text_area.y-TOP_MARGIN,
             mLeftMargin, text_area.h+TOP_MARGIN+BOTTOM_MARGIN, color());
    fl_rectf(text_area.x+text_area.w, text_area.y,
             RIGHT_MARGIN, text_area.h, color());
    fl_pop_clip();
  }
...
// this processes drag events due to mouse for My_Text_Display and
// also drags due to cursor movement with shift held down for
// Fl_Text_Editor
void fl_text_drag_me(int pos, My_Text_Display* d) {
  if (d->dragType == My_Text_Display::DRAG_CHAR) {
    if (pos >= d->dragPos) {
  ...
  } else if (d->dragType == My_Text_Display::DRAG_WORD) {
    if (pos >= d->dragPos) {
  ...
  } else if (d->dragType == My_Text_Display::DRAG_LINE) {
    if (pos >= d->dragPos) {
  ...
  switch (event) {
    ...
    case FL_MOVE:
      if (active_r()) {
		...
		else if ((Fl::event_x() < text_area.x) && (Fl::event_y() >= text_area.y) &&
			(Fl::event_y() <= text_area.y + text_area.h))
    ...
    case FL_PUSH: {
		if ((mVScrollBar->visible() && Fl::event_x() > text_area.x + text_area.w))
			break;
		if ((mHScrollBar->visible() && Fl::event_y() > text_area.y + text_area.h))
			break;
	  if (Fl::focus() != this) {
	    Fl::focus(this);
	    handle(FL_FOCUS);
		...
		{
		  buffer()->secondary_unselect();
		  position_to_linecol(pos, &line, &mCurrentCol);
		  if (strcmp(buffer()->text_range(buffer()->line_start(pos), buffer()->line_start(pos) + 14), "Error in line ") == 0)
		  {
			  /* Determine line number and filename of error */
			  int errNumPos = buffer()->word_start(buffer()->line_start(pos) + 14);
			  int errNumEndPos = buffer()->word_end(errNumPos);
			  ...
			  {
		          buffer()->secondary_select(buffer()->line_start(pos), buffer()->line_end(pos)+1);
				  redraw();
				  m_pErrorFunc(errLine, pFile, m_pErrorOpaque);
				  /* Perform Secondary selection on this line */
				  return 1;
			  }
		  }
		  else
